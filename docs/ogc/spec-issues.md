...
| SI-24 | L | `distributing-geoparquet.md` L21 vs L92 vs L98; `gpio check` | Row-group guidance disagrees with itself: the tl;dr says 50 000–150 000 rows, the body says 50 000–200 000 rows and 128–256 MB, `gpio check row-group` reports 64–256 MB as optimal and `gpio check optimization` scores 10 000–50 000 rows as optimal for spatial pushdown. The proposed Cloud-Optimized Distribution class needs one agreed range. | Pick one set of numbers (or state that rows and bytes are alternatives) in the guide and in `gpio`. |
