...
| SI-26 | M | `geoparquet.md` (PR #302) "Bounding Box Columns": "the child fields MUST be named `xmin`, `ymin`, `xmax`, and `ymax` and MUST be ordered in this same way" (same words in 1.1.0) | Every Overture Maps release (2026-08-19.0 checked, all themes) writes the bbox group as `xmin, xmax, ymin, ymax`. No validator has ever checked the order (gpio checks names only), so the largest GeoParquet producer has been non-conformant with 1.1 and would fail `/conf/covering/bbox-column-structure`. Parquet readers address struct fields by name; the order carries no information. | Drop "and MUST be ordered in this same way" in #302 (and the corresponding part of `/req/covering/bbox-column-structure`), or tell Overture. Found by the Rust checker in geoparquet-testing#6. |
...
| SI-30 | L | `geoparquet.md` L91 (`srid:0` SHOULD) vs `/req/core/crs-consistency` | The spec only *SHOULD*s `srid:0` when the GeoParquet `crs` is `null`, but the OGC requirement that the two CRS statements agree makes `null` with an unset Parquet `crs` (= CRS84) a failure. Deliberate in the OGC draft; the community text is softer. | Either make L91 a MUST in 2.0.0 or note the difference in CHANGES.md (done). |
